...
function makeMockService(overrides?: Partial<IWeatherService>): IWeatherService {
  return {
    name: 'MockService',
...
describe('useWeather', () => {
  it('returns null weather and no errors before a location is entered', () => {
    const { result } = renderHook(() => useWeather(makeMockService()), { wrapper: makeWrapper() });
  ...
  it('fetches weather for a valid location', async () => {
    ...
    const { result } = renderHook(() => useWeather(service), { wrapper: makeWrapper() });
  ...
  it.each([
    ['NOT_FOUND',           new WeatherServiceError('not found', 'NOT_FOUND'),           'Location "London" not found.'],
    ['NETWORK',             new WeatherServiceError('network error', 'NETWORK'),          'Check your connection.'],
    ['SERVICE_UNAVAILABLE', new WeatherServiceError('unavailable', 'SERVICE_UNAVAILABLE'),'Service unavailable. Please try again.'],
    ['unexpected error',    new Error('unexpected'),                                       'Something went wrong. Please try again.'],
  ])('maps %s to the correct fetchError message', async (_, error, expected) => {
    const service = makeMockService({ fetchWeather: jest.fn().mockRejectedValue(error) });
    const { result } = renderHook(() => useWeather(service), { wrapper: makeWrapper() });
    await act(async () => result.current.setLocation('London'));
