...
describe('mapOpenMeteoResponse', () => {
  it('maps raw response to WeatherData', () => {
    const result = mapOpenMeteoResponse({ name: 'London', current: { temperature_2m: 18.5, weathercode: 1 } });
...
describe('OpenMeteoService.fetchWeather', () => {
  ...
  it('returns mapped WeatherData on success', async () => {
    (global as any).fetch
      ...
      .mockResolvedValueOnce({ ok: true, json: async () => baseForecastResponse });
  ...
  it('throws WeatherServiceError with NOT_FOUND when geocoding returns no results', async () => {
    (global as any).fetch.mockResolvedValueOnce({ ok: true, json: async () => ({ results: [] }) });
